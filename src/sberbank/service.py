...
from sberbank.exceptions import NetworkException, ProcessingException, PaymentNotFoundException
...
class BankService(object):
    ...
    def _get_credentials(self, merchant_id):
        ...
        if merchants is None:
            raise KeyError("Key %s not found in settings.py" % settings_merchant_key)

        self.merchant = merchants.get(merchant_id, None)
        if self.merchant is None:
            raise KeyError("Merchant key %s not found in %s" % (merchant_id, settings_merchant_key))

        for field_name in ["username", "password"]:
            if self.merchant.get(field_name, None) is None:
                raise KeyError("Field '%s' not found in %s->%s" % (field_name, settings_merchant_key, merchant_id))
    ...
    def mobile_pay(self, amount, token, ip, **kwargs):
        ...
        try:
            ...
            raise TypeError("Wrong amount type, passed {} ({}) instead of decimal".format(amount, type(amount)))

        payment = Payment(
            amount=amount,
            client_id=client_id,
            method=db_method,
            details={"username": self.merchant.get("username"), "currency": currency},
        )
        ...
        data = {"merchant": self.merchant_id, "orderNumber": payment.uid.hex, "paymentToken": token, "ip": ip}
        if method == "google/payment":
            data.update({"amount": int(amount * 100), "returnUrl": success_url, "failUrl": fail_url})
        if kwargs.get("params"):
    ...
    def execute_request(self, data, method, payment=None):
        ...
        if rest:
            data.update({"userName": self.merchant.get("username"), "password": self.merchant.get("password")})
        else:
        ...
        if response.status_code != 200:
            if payment:
                payment.status = Status.FAILED
                payment.save()
            raise ProcessingException(payment.uid if payment else None, response.text, response.status_code)
        try:
        ...
        if int(response.get("errorCode", 0)) != 0:
            ...
            raise ProcessingException(
                payment.uid if payment else None, response.get("errorMessage"), response.get("errorCode")
            )
